...
func Values_RMUSR(instructions []string) (string, bool) {
	...
	for _, valor := range instructions {
		if strings.HasPrefix(strings.ToLower(valor), "user") {
			var value = utils.TieneUser("RMUSR", valor)
			if len(value) > 10 {
...
func RMUSR_EXECUTE(_user string) {
	if !global.UsuarioLogeado.Logged_in {
		color.Red("[RMUSR]: No hay usuario loggeado")
		return
	}
	if global.UsuarioLogeado.UID != 1 && global.UsuarioLogeado.GID != 1 {
		color.Red("[RMUSR]: No tienes permisos para ejecutar este comando")
		return
	}
	...
	if err != nil {
		color.Red("[RMUSR]: Error al abrir archivo")
		return
	}
	defer file.Close()
	...
	if mount.Es_Particion_L {
		...
		if _, err := file.Seek(int64(inicio), 0); err != nil {
			color.Red("[RMUSR]: Error en mover puntero")
			return
		}
		if err := binary.Read(file, binary.LittleEndian, &utils.Sb_AdminUsr); err != nil {
			color.Red("[RMUSR]: Error en la lectura del SuperBloque")
			return
		}
	...
	if utils.UsrExist(split_content, _user) {
			...
			if econ != "" {
				if strings.Contains(econ, ",U,") || strings.Contains(econ, ",G,") {
					if strings.Contains(econ, ",U,") {
						if strings.Contains(econ, _user) {
							continue
						}
					}
		...
		if err := binary.Read(file, binary.LittleEndian, &inodo); err != nil {
			color.Red("[RMUSR]: Error en la lectura del Inodo")
			return
		}
		...
		if len(usersTxt_Old) > len(usersTxt) {
			...
			for i := start; i < end; i++ {
				...
				if err := binary.Read(file, binary.LittleEndian, &bit); err != nil {
					color.Red("[RMUSR]: Error en la lectura del Archivo")
					return
				}
				if bit == '1' { //ocupado
					cantContiguos = 0
			...
			if err := binary.Write(file, binary.LittleEndian, &zero); err != nil {
				color.Red("[RMUSR]: Error en la escritura del Archivo")
				return
			}
		...
		if err := binary.Write(file, binary.LittleEndian, &inodo); err != nil {
			color.Red("[RMUSR]: Error en la escritura del Inodo")
			return
		}
		...
		if mount.Es_Particion_P {
			...
			if err := binary.Write(file, binary.LittleEndian, &utils.Sb_AdminUsr); err != nil {
				color.Red("[RMUSR]: Error en la escritura del SuperBloque")
				return
			}
		...
		if utils.Sb_AdminUsr.S_filesistem_type == 3 {
			//escribimos journal
			utils.EscribirJournal("rmusr", '1', "users.txt", _user)
		}

	} else {
		color.Red("[RMUSR]: El usuario «" + _user + "» no existe")
		return
	}
	color.Blue("[RMUSR]: Usuario «" + _user + "» eliminado correctamente")
}
