...
func EscribirJournal(tipoOp string, tipo byte, nombre string, contenido string) {
	...
	if mount.Es_Particion_P {
		if _, err := file.Seek(int64(mount.Particion_P.Part_start+size.SizeSuperBloque()), 0); err != nil {
			color.Red("[-]: Error en mover puntero")
			return
		}
	} else if mount.Es_Particion_L {
		if _, err := file.Seek(int64(mount.Particion_L.Part_start+size.SizeEBR()+size.SizeSuperBloque()), 0); err != nil {
			color.Red("[-]: Error en mover puntero")
			return
		}
	}
	...
	if (actJour.J_Start + size.SizeJournal()) > Sb_AdminUsr.S_bm_inode_start {
		return
	}
