...
func Values_MKGRP(instructions []string) (string, bool) {
	...
	for _, valor := range instructions {
		if strings.HasPrefix(strings.ToLower(valor), "name") {
			var value = utils.TieneNombre("MKGRP", valor)
			if len(value) > 10 {
...
func MKGRP_EXECUTE(_name string) {
	...
	if global.UsuarioLogeado.UID != 1 && global.UsuarioLogeado.GID != 1 {
		color.Red("[MKGRP]: No tienes permisos para ejecutar este comando")
		return
	}
	...
	if !utils.GrupoExist(split_content, _name) {
		...
		if len(usersTxt) > 4380 {
			color.Red("[MKGRP]: No se pueden crear más grupos")
			return
		}

		if utils.Sb_AdminUsr.S_free_blocks_count < (cantBlockAct - cantBlockAnt) {
			color.Red("[MKGRP]: No hay bloques suficientes para crear archivo")
			return
		}
		...
		if (cantBlockAct - cantBlockAnt) > 0 {
			for i := start; i < end; i++ {
				...
				if err := binary.Read(file, binary.LittleEndian, &bit); err != nil {
					color.Red("[MKGRP]: Error en la lectura del Archivo")
					return
				}
				if bit == '1' { //ocupado
					cantContiguos = 0
			...
			if (inicioBM == -1) || (cantContiguos != (cantBlockAct - cantBlockAnt)) {
				color.Red("[MKGRP]: No hay bloques suficientes para actualizar el archivo")
				return
			}
		...
		if utils.Sb_AdminUsr.S_filesistem_type == 3 {
			//escribimos journal
			utils.EscribirJournal("mkgrp", '1', "users.txt", nuevoGrp)
		}
	} else {
		color.Red("[MKGRP]: El grupo «" + _name + "» ya existe")
		return
	}

	color.Green("[MKGRP]: Grupo «" + _name + "» guardado correctamente")
